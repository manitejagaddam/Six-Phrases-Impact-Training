...
class polyDivisible
{
    public static long anyBaseConverter (String strval, int base )
	{
     ...
     private static void polyDivisible_solver_helper( 
	   int [] used, int radix, StringBuilder res, String pd, int pd_ind )
	   {
	       if ( pd_ind == radix )
		   {
		  ...
		  String test;
		  char ch;
		  for( work_pd_ind = 0; work_pd_ind < radix ; work_pd_ind ++)
		  {
		     if ( used [ work_pd_ind] == 1)
			      continue;
		     test = res.toString() + pd.charAt( work_pd_ind );
             long testValue;
			 ...
			 {
			     used [ work_pd_ind ] = 1;
				 res.append( pd.charAt ( work_pd_ind ));
				 polyDivisible_solver_helper ( used, radix, res, pd, pd_ind+1);
     ...
     public static void polyDivisible_solver( int base )
	 {
	   ...
	   if ( base % 2 != 0)
		...
		polyDivisible_solver_helper ( used, base, res, possible_digits, 1 );
